...
import type { CustomLineChartProps, ChartDataPoint, ChartDataItem } from "../types";
...
const CustomLineChart = ({ data }: CustomLineChartProps) => {
    const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: Array<{ payload: ChartDataPoint }>; label?: string }) => {
        if (active && payload && payload.length) {
            ...
            const groupedItemsForTooltip = dataPoint.items.reduce<Record<string, { categoryName: string; totalAmount: number }>>((acc, item: ChartDataItem) => {
